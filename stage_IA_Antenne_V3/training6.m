    function [netfin, trfin, y2_Train, y2_TrainTrue, y2_Val, y2_ValTrue, RMSEtrain, RMSEval, n] = training1(bd, couche, epochs, ptrain, pval, ptest, mode)
    %% Normalisation des données (H,Er,Fr)->(Wp,Lp,Fi,Wf,Gpf,Lg,Wg)
    ...
    %% Entrainement du réseau de neuronne (H,Er,Fr)->(Wp,Lp,Fi,Wf,Gpf,Lg,Wg)
    if(mode == false)    
        n = permR(couche);
    else
        n=couche;
    ...
    RMSEtrain = zeros(7,size(n,1));
    RMSEval = zeros(7,size(n,1));
