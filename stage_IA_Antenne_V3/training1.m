    function [netfin, trfin, y2_Train, y2_TrainTrue, y2_Val, y2_ValTrue, RMSEtrain, RMSEval, n] = training1_2Outputs(bd, couche, epochs, ptrain, pval, ptest, mode)
    %% Normalisation des données (H,Er,Fr)->(Wp,Lp)
    ...
    %% Entrainement du réseau de neuronne (H,Er,Fr)->(Wp,Lp)
    if(mode == false)    
        n = permR(couche);
    else
        n=couche;
